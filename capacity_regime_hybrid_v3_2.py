# ============================================================
# Capacity-Regime Dark Matter Model — Hybrid v3.2
#   - Sofue-style MW baryons
#   - Two-term Pi_O(r)
#   - Extended E(r) with halo taper
#   - Free alpha, epsilon_E, r_E
#   - Physical + soft priors
#   - Nelder–Mead χ² fit
#   - χ² heatmaps
...
# ============================================================
# Hybrid capacity-regime fields (with halo taper)
# ============================================================
...
def apply_physical_priors(params):
    ...
    #           log_eta, log_r_H, log_p, log_r_T,
    #           log_epsE, log_r_E)
    (log_rho_star, log_Pi0, log_r_P,
     log_Pi1, log_r_Q, log_alpha,
     log_eta, log_r_H, log_p, log_r_T,
     log_epsE, log_r_E) = params
    ...
    if not (0.1 <= Pi0 <= 10.0):
        penalty += 1e6
    if not (0.05 <= Pi1 <= 10.0):
        penalty += 1e6

    # Coherence band for capacity scales
    if not (2.0 <= r_P <= 30.0):
        penalty += 1e6
    if not (2.0 <= r_Q <= 40.0):
        penalty += 1e6
    ...
    if not (2.0 <= r_H <= 80.0):
        penalty += 1e6
    if not (5.0 <= r_T <= 100.0):
        penalty += 1e6

    # Expansion inner scale
    if not (1.0 <= r_E <= 20.0):
        penalty += 1e6

    # alpha broad, but with soft preference
    if not (0.5 <= alpha <= 6.0):
        penalty += 1e6
    ...
    # epsE in [0.05, 2]
    if not (0.05 <= epsE <= 2.0):
        penalty += 1e6

    # Soft prior: alpha prefers ~2–3
    if alpha > 3.0:
        penalty += 1e4 * (alpha - 3.0)**2

    # Soft prior: r_T not much smaller than r_H
    if r_T < 0.5 * r_H:
        penalty += 5e5

    return penalty

def mw_chi2_params_hybrid_v32(params, r, r_data, v_data, v_err):
...
def fit_mw_parameters_hybrid_v32(r, r_data, v_data, v_err):
...
def _E_kwargs_from_params(params, dwarf=False):
    if dwarf:
        E_bg = 1.1
    else:
        E_bg = 1.0
...
def _Pi_kwargs_from_params(params, dwarf=False):
    if dwarf:
        return {
            "Pi0": 0.3 * params["Pi0"],
            "r_P": 0.5 * params["r_P"],
...
def plot_mw_with_data_hybrid_v32(r, r_data, v_data, v_err,
                                 params,
                                 save_name="mw_capacity_hybrid_v32_fit.png",
                                 title_suffix=""):

    E_kwargs = _E_kwargs_from_params(params, dwarf=False)
    Pi_kwargs = _Pi_kwargs_from_params(params, dwarf=False)
    ...
    plt.ylabel("v_c [km/s]")
    plt.legend()
    plt.title(f"Milky Way — Hybrid v3.2 {title_suffix}")
    plt.tight_layout()
    plt.savefig(save_name, dpi=300)
    plt.show()

def plot_dwarf_with_data_hybrid_v32(r, r_data, v_data, v_err,
                                    params,
                                    save_name="draco_capacity_hybrid_v32.png",
                                    dwarf_name="Draco"):

    E_kwargs = _E_kwargs_from_params(params, dwarf=True)
    Pi_kwargs = _Pi_kwargs_from_params(params, dwarf=True)
    ...
    plt.ylabel("v_c [km/s]")
    plt.legend()
    plt.title(f"{dwarf_name} — Hybrid v3.2")
    plt.tight_layout()
    plt.savefig(save_name, dpi=300)
    plt.show()
...
def mw_chi2_heatmap_Pi_rP_v32(r, r_data, v_data, v_err,
    ...
    plt.ylabel(r"$\Pi_0$")
    plt.title("MW χ² landscape — (Pi0, r_P), v3.2")
    plt.tight_layout()
    plt.savefig(save_name, dpi=300)
    plt.show()

def mw_chi2_heatmap_eta_rH_v32(r, r_data, v_data, v_err,
                               base_params,
                               eta_grid=np.linspace(0.0, 3.0, 30),
                               rH_grid=np.linspace(5.0, 40.0, 30),
                               save_name="mw_chi2_eta_rH_v32.png"):
    ...
    for i, eta in enumerate(eta_grid):
        for j, r_H in enumerate(rH_grid):
            E_kwargs = {
                "E_bg": 1.0,
                "epsilon_E": base_params["epsilon_E"],
                "r_E": base_params["r_E"],
                "eta": eta,
                "r_H": r_H,
                "p": base_params["p"],
            ...
            Pi_kwargs = _Pi_kwargs_from_params(base_params, dwarf=False)
    ...
    plt.ylabel(r"$\eta$")
    plt.title("MW χ² landscape — (eta, r_H), v3.2")
    plt.tight_layout()
    plt.savefig(save_name, dpi=300)
    plt.show()
...
def auto_stress_test_v32(r, out_dir="stress_plots_v32"):
                        ...
                        for r_Q in r_Q_values:
                            for r_T in r_T_values:
                                ...
                                plt.title(
                                    f"a={alpha}, eta={eta}, "
                                    f"Pi0={Pi0}, Pi1={Pi1}, "
                                    f"r_P={r_P}, r_Q={r_Q}, r_T={r_T}"
                                )
                                plt.tight_layout()

                                fname = (
                                    f"stress_v32_a{alpha}_eta{eta}_Pi0{Pi0}_Pi1{Pi1}_"
                                    f"rP{r_P}_rQ{r_Q}_rT{r_T}_{idx}.png"
                                )
...
def auto_run_mw_v32():
    ...
    print("🔧 Fitting hybrid v3.2 capacity-regime parameters (Nelder–Mead χ²)...")
    best_params, chi2_best = fit_mw_parameters_hybrid_v32(
        r, r_data, v_data, v_err
    )

    print("✅ Best-fit parameters (hybrid v3.2):")
    for k, v in best_params.items():
        print(f"   {k} = {v:.4g}")
    print(f"   χ² = {chi2_best:.2f}")

    print("📊 Generating paper-ready MW figure (v3.2)...")
    plot_mw_with_data_hybrid_v32(
        r,
        r_data,
        v_data,
        v_err,
        params=best_params,
        save_name="mw_capacity_hybrid_v32_fit.png",
        title_suffix="(hybrid + Sofue + halo taper)",
    )

    print("🗺 Generating MW χ² heatmap (Pi0, r_P) v3.2...")
    mw_chi2_heatmap_Pi_rP_v32(
        r,
        r_data,
        v_data,
        v_err,
    ...
    print("🗺 Generating MW χ² heatmap (eta, r_H) v3.2...")
    mw_chi2_heatmap_eta_rH_v32(
        r,
        r_data,
        v_data,
        v_err,
...
def auto_run_dwarf_v32(best_params):
    ...
    print("📊 Generating Draco hybrid v3.2 figure...")
    plot_dwarf_with_data_hybrid_v32(
        r,
        r_data,
        v_data,
        v_err,
...
def auto_run_stress_v32():
    print("🧪 Running hybrid v3.2 stress-test sweep (this may take a while)...")
    auto_stress_test_v32(r, out_dir="stress_plots_v32")
    print("✅ Stress-test plots saved in ./stress_plots_v32")
...
if __name__ == "__main__":
    best_params = auto_run_mw_v32()
    auto_run_dwarf_v32(best_params)
    auto_run_stress_v32()
    print("🎯 Hybrid v3.2 auto-run complete.")
