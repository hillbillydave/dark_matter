# ============================================================
# Capacity-Regime Dark Matter Model — Hybrid v3.1
...
def E_field(r, E_bg=1.0, epsilon_E=0.3, r_E=10.0,
            ...
            r_T=30.0):
    # Extended expansion field with halo-like growth and exponential taper
    return E_bg * (
        1.0
...
def apply_physical_priors(params):
    ...
    (log_rho_star, log_Pi0, log_r_P,
     log_Pi1, log_r_Q, log_alpha,
     log_eta, log_r_H, log_p, log_r_T) = params
    ...
    # r_P, r_Q, r_H, r_T in [0.5, 60] kpc
    for val in (r_P, r_Q, r_H, r_T):
        if not (0.5 <= val <= 60.0):
            penalty += 1e6
    ...
    # r_T should not be much smaller than r_H (soft prior)
    if r_T < 0.5 * r_H:
        penalty += 5e5

    return penalty

def mw_chi2_params_hybrid_v31(params, r, r_data, v_data, v_err):
...
def fit_mw_parameters_hybrid_v31(r, r_data, v_data, v_err):
    ...
    (log_rho_star, log_Pi0, log_r_P,
     log_Pi1, log_r_Q, log_alpha,
     log_eta, log_r_H, log_p, log_r_T) = res.x
...
def plot_mw_with_data_hybrid_v31(r, r_data, v_data, v_err,
                                 params,
                                 save_name="mw_capacity_hybrid_v31_fit.png",
                                 title_suffix=""):
    ...
    plt.ylabel("v_c [km/s]")
    plt.legend()
    plt.title(f"Milky Way — Hybrid v3.1 {title_suffix}")
    plt.tight_layout()
    plt.savefig(save_name, dpi=300)
    plt.show()

def plot_dwarf_with_data_hybrid_v31(r, r_data, v_data, v_err,
                                    params,
                                    save_name="draco_capacity_hybrid_v31.png",
                                    dwarf_name="Draco"):

    # Simple scaling: dwarfs live in lower-capacity, slightly higher-expansion environments
    E_kwargs = {
        "E_bg": 1.1,
        "epsilon_E": 0.2,
        "r_E": 10.0,
    ...
    plt.ylabel("v_c [km/s]")
    plt.legend()
    plt.title(f"{dwarf_name} — Hybrid v3.1")
    plt.tight_layout()
    plt.savefig(save_name, dpi=300)
    plt.show()
...
def mw_chi2_heatmap_Pi_rP_v31(r, r_data, v_data, v_err,
                               base_params,
                               Pi0_grid=np.linspace(0.3, 3.0, 30),
                               rP_grid=np.linspace(2.0, 20.0, 30),
                               save_name="mw_chi2_Pi_rP_v31.png"):
    ...
    plt.ylabel(r"$\Pi_0$")
    plt.title("MW χ² landscape — (Pi0, r_P), v3.1")
    plt.tight_layout()
    plt.savefig(save_name, dpi=300)
    plt.show()

def mw_chi2_heatmap_eta_rH_v31(r, r_data, v_data, v_err,
                               base_params,
                               eta_grid=np.linspace(0.0, 3.0, 30),
                               rH_grid=np.linspace(5.0, 40.0, 30),
                               save_name="mw_chi2_eta_rH_v31.png"):
    ...
    plt.ylabel(r"$\eta$")
    plt.title("MW χ² landscape — (eta, r_H), v3.1")
    plt.tight_layout()
    plt.savefig(save_name, dpi=300)
    plt.show()
...
def auto_stress_test_v31(r, out_dir="stress_plots_v31"):
                    ...
                    for r_P in r_P_values:
                        for r_Q in r_Q_values:
                            for r_T in r_T_values:
                                ...
                                fname = (
                                    f"stress_v31_a{alpha}_eta{eta}_Pi0{Pi0}_Pi1{Pi1}_"
...
def auto_run_mw_v31():
    ...
    print("🔧 Fitting hybrid v3.1 capacity-regime parameters (Nelder–Mead χ²)...")
    best_params, chi2_best = fit_mw_parameters_hybrid_v31(
        r, r_data, v_data, v_err
    )

    print("✅ Best-fit parameters (hybrid v3.1):")
    for k, v in best_params.items():
        print(f"   {k} = {v:.4g}")
    print(f"   χ² = {chi2_best:.2f}")

    print("📊 Generating paper-ready MW figure (v3.1)...")
    plot_mw_with_data_hybrid_v31(
        r,
        r_data,
        v_data,
        v_err,
    ...
    print("🗺 Generating MW χ² heatmap (Pi0, r_P) v3.1...")
    mw_chi2_heatmap_Pi_rP_v31(
        r,
        r_data,
        v_data,
        v_err,
    ...
    print("🗺 Generating MW χ² heatmap (eta, r_H) v3.1...")
    mw_chi2_heatmap_eta_rH_v31(
        r,
        r_data,
        v_data,
        v_err,
...
def auto_run_dwarf_v31(best_params):
    ...
    print("📊 Generating Draco hybrid v3.1 figure...")
    plot_dwarf_with_data_hybrid_v31(
        r,
        r_data,
        v_data,
        v_err,
...
def auto_run_stress_v31():
    print("🧪 Running hybrid v3.1 stress-test sweep (this may take a while)...")
    auto_stress_test_v31(r, out_dir="stress_plots_v31")
    print("✅ Stress-test plots saved in ./stress_plots_v31")
...
if __name__ == "__main__":
    best_params = auto_run_mw_v31()
    auto_run_dwarf_v31(best_params)
    auto_run_stress_v31()
    print("🎯 Hybrid v3.1 auto-run complete.")
